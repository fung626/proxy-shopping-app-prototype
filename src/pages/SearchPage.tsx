...
import { ArrowLeft, Search, Filter, MapPin, Star, Heart, Clock, Package, Users } from 'lucide-react';
...
export function SearchPage({ 
  ...
  const allResults = [
    // Requests
    {
      ...
      offers: 12,
      category: 'fashion',
      image: 'https://images.unsplash.com/photo-1645633090316-bcaafbfd4c4d?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxzaGVycGElMjBjb2F0JTIwd2ludGVyJTIwamFja2V0fGVufDF8fHx8MTc1ODcxMjE3Nnww&ixlib=rb-4.1.0&q=80&w=1080'
    },
    {
      id: 2,
      type: 'request' as const,
      ...
      image: 'https://images.unsplash.com/photo-1667178173387-7e0cb51c0b4f?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHx3aXJlbGVzcyUyMGVhcmJ1ZHMlMjBoZWFkcGhvbmVzfGVufDF8fHx8MTc1ODYwNjc1MXww&ixlib=rb-4.1.0&q=80&w=1080'
    },
    // Offers
    {
      id: 3,
      ...
      category: 'beauty',
      image: 'https://images.unsplash.com/photo-1686831451322-8d8e234a51e1?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxrb3JlYW4lMjBza2luY2FyZSUyMGJlYXV0eSUyMHByb2R1Y3RzfGVufDF8fHx8MTc1ODcwNTYxOXww&ixlib=rb-4.1.0&q=80&w=1080',
    ...
    },
    {
      ...
      description: 'Latest tech gadgets from Tokyo electronics district',
      price: 299.99,
      currency: 'USD',
      ...
      category: 'electronics',
      image: 'https://images.unsplash.com/photo-1584658645175-90788b3347b3?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxqYXBhbmVzZSUyMGVsZWN0cm9uaWNzJTIwdGVjaCUyMGdhZGdldHN8ZW58MXx8fHwxNzU4NzA1NjIxfDA&ixlib=rb-4.1.0&q=80&w=1080',
    ...
    },
    {
      ...
      category: 'fashion',
      image: 'https://images.unsplash.com/photo-1575404199108-c7417489517d?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxmYXNoaW9uJTIwYWNjZXNzb3JpZXMlMjBwYXJpcyUyMGx1eHVyeXxlbnwxfHx8fDE3NTg3MDU2MjR8MA&ixlib=rb-4.1.0&q=80&w=1080',
    ...
    },
    {
      ...
      image: 'https://images.unsplash.com/photo-1664764966090-e5d93ae48dc2?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxsdXh1cnklMjB0b3RlJTIwYmFnJTIwaGFuZGJhZ3xlbnwxfHx8fDE3NTg3MTIxODJ8MA&ixlib=rb-4.1.0&q=80&w=1080'
    }
  ];
  ...
  const filteredResults = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      results = results.filter(item => 
        ...
        (item.type === 'request' && item.user.toLowerCase().includes(searchQuery.toLowerCase())) ||
        (item.type === 'offer' && item.agentName.toLowerCase().includes(searchQuery.toLowerCase()))
      );
    }
    ...
    switch (sortBy) {
      case 'newest':
        results = results.sort((a, b) => {
          const timeA = a.type === 'request' ? new Date(a.timeAgo) : new Date();
          const timeB = b.type === 'request' ? new Date(b.timeAgo) : new Date();
          return timeB.getTime() - timeA.getTime();
        });
        break;
      case 'price-low':
        results = results.sort((a, b) => {
          const priceA = a.type === 'request' ? a.budget.min : a.price;
          const priceB = b.type === 'request' ? b.budget.min : b.price;
          return priceA - priceB;
        });
        break;
      case 'price-high':
        results = results.sort((a, b) => {
          const priceA = a.type === 'request' ? a.budget.max : a.price;
          const priceB = b.type === 'request' ? b.budget.max : b.price;
          return priceB - priceA;
        });
        break;
      case 'popular':
        results = results.sort((a, b) => {
          const popularityA = a.type === 'request' ? a.offers : a.agentReviews;
          const popularityB = b.type === 'request' ? b.offers : b.agentReviews;
      ...
      <div className="bg-card px-4 pt-8 pb-4 border-b border-border">
        ...
        <div className="flex items-center space-x-3">
          ...
          <Button 
            ...
            className="h-9 px-3 bg-input-background border-0 rounded-lg"
          >
      ...
      <div className="px-4 py-6">
        <div className="mb-4">
          <span className="text-muted-foreground text-sm">
            {t('common.itemsFound', { count: filteredResults.length })}
          </span>
        </div>
        ...
        {filteredResults.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-muted-foreground">
              {searchQuery ? t('common.adjustSearchTerms') : t('search.tryDifferentTerms')}
            </p>
          </div>
        ) : (
                ...
                <div className="relative mb-3">
                  ...
                  {item.type === 'request' ? (
                    <Badge className="absolute top-2 left-2 bg-blue-600 text-white text-xs px-2 py-1">
                      {item.budget.currency === 'USD' ? '$' : item.budget.currency === 'EUR' ? '€' : '¥'}{item.budget.min}-{item.budget.max}
                    </Badge>
                  ) : (
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    ...
                    {item.type === 'request' ? (
                      ...
                      <div className="flex items-center space-x-1">
                        ...
                        <span className="whitespace-nowrap">{item.agentRating}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-muted-foreground truncate mr-2">
                      {item.type === 'request' ? item.user : item.agentName}
                    </span>
                    {item.type === 'request' ? (
                      <div className="flex items-center space-x-1">
