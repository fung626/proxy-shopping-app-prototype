...
export function ChangePasswordPage({ onBack, onComplete }: ChangePasswordPageProps) {
  const { t } = useLanguage();
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="p-4">
        <div className="mb-6">
          ...
          <h2 className="text-xl font-semibold mb-2">{t('changePassword.updatePassword')}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            {newPassword && newPassword.length < 8 && (
              <p className="text-sm text-destructive">{t('changePassword.requirement1')}</p>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            {confirmPassword && newPassword !== confirmPassword && (
              <p className="text-sm text-destructive">{t('changePassword.passwordsDoNotMatch')}</p>
            )}
          </div>
          ...
          <div className="p-4 bg-muted/50 rounded-lg">
            <h4 className="font-medium mb-2">{t('changePassword.passwordRequirements')}:</h4>
            <ul className="text-sm text-muted-foreground space-y-1">
              <li className={`flex items-center space-x-2 ${newPassword.length >= 8 ? 'text-green-600' : ''}`}>
                <div className={`w-1.5 h-1.5 rounded-full ${newPassword.length >= 8 ? 'bg-green-600' : 'bg-muted-foreground'}`} />
          ...
          >
            {isLoading ? t('changePassword.updating') : t('changePassword.changeButton')}
          </Button>
        </form>
      </div>
    </div>
