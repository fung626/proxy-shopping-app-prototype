...
export function PhoneVerificationPage({ onBack, onComplete, phone = '' }: PhoneVerificationPageProps) {
  const { t } = useLanguage();
  const [step, setStep] = useState(phone ? 2 : 1); // Skip phone input if provided
  const [phoneNumber, setPhoneNumber] = useState(phone ? phone.replace(/^\+\d+/, '') : ''); // Extract phone without country code
  const [countryCode, setCountryCode] = useState(phone ? phone.match(/^\+\d+/)?.[0] || '+1' : '+1'); // Extract country code or default
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="px-4 py-4 safe-area-inset-top">
        <div className="flex items-center">
          ...
          <h1 className="text-lg font-semibold text-foreground">{t('phoneVerification.title')}</h1>
        </div>
      </div>
          ...
          {step === 1 ? (
            ...
            <div>
              <div className="text-center mb-8">
                ...
                <h2 className="text-2xl font-semibold text-foreground mb-3">{t('phoneVerification.verifyYourPhone')}</h2>
              ...
              <div className="space-y-4 mb-8">
                <div>
                  <Label htmlFor="phone">{t('phoneVerification.phoneLabel')}</Label>
                  <PhoneInput
            ...
            <div>
              <div className="text-center mb-8">
                ...
                <h2 className="text-2xl font-semibold text-foreground mb-3">{t('phoneVerification.enterVerificationCode')}</h2>
                <p className="text-muted-foreground leading-relaxed">
                  {t('phoneVerification.codeDescription', { phone: formatPhoneNumber(countryCode, phoneNumber) })}
                </p>
              </div>

              <div className="space-y-6 mb-8">
                <div>
                  <Label htmlFor="code">{t('phoneVerification.verificationCodeLabel')}</Label>
                  <Input
                    id="code"
                    type="text"
                ...
                <div className="text-center">
                  <p className="text-sm text-muted-foreground mb-3">
                    ...
                    {resendCount > 0 && ` ${t('phoneVerification.sentCodeCount', { count: resendCount, plural: resendCount > 1 ? 's' : '' })}`}
                  </p>
                  <Button
                    variant="outline"
                    size="sm"
                  ...
                  >
                    {isResending ? (
                      ...
                      t('phoneVerification.resendInSeconds', { seconds: cooldown })
                    ) : (
                ...
                <div className="bg-muted/50 rounded-lg p-4">
                  <h4 className="font-medium text-foreground mb-2">{t('phoneVerification.smsTips')}</h4>
