...
export function AddCreditCardPage({ onBack, onSave, user }: AddCreditCardPageProps) {
  const { t } = useLanguage();
  ...
  const cardTypeFromNumber = (number: string): CreditCardInfo['cardType'] => {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="sticky top-0 bg-background border-b border-border px-4 py-3">
        <div className="flex items-center">
          ...
          <h1 className="text-lg font-semibold">{t('addCreditCard.title')}</h1>
        </div>
      </div>
      ...
      <div className="p-4">
        <div className="mb-6">
          ...
          <h2 className="text-xl font-semibold mb-2">{t('addCreditCard.subtitle')}</h2>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            <Label htmlFor="cardNumber">{t('addCreditCard.cardNumber')} *</Label>
            <div className="relative">
              ...
              <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center space-x-2">
                {cardData.cardNumber && getCardIcon(cardTypeFromNumber(cardData.cardNumber))}
                <Button
                  type="button"
                  variant="ghost"
                  size="sm"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              <Label htmlFor="expiryDate">{t('addCreditCard.expiryDate')} *</Label>
              <Input
                id="expiryDate"
          ...
          {/* Cardholder Name */}
          <div className="space-y-2">
            <Label htmlFor="cardholderName">{t('addCreditCard.cardholderName')} *</Label>
            <Input
              id="cardholderName"
          ...
          {/* Billing Email */}
          <div className="space-y-2">
            <Label htmlFor="billingEmail">{t('addCreditCard.billingEmail')} *</Label>
            <Input
              id="billingEmail"
              type="email"
              placeholder={t('addCreditCard.billingEmailPlaceholder')}
              value={cardData.billingEmail || ''}
              onChange={(e) => setCardData(prev => ({ ...prev, billingEmail: e.target.value }))}
              required
            />
          </div>
          ...
          {/* Card Nickname */}
          <div className="space-y-2">
            <Label htmlFor="nickname">{t('addCreditCard.nickname')}</Label>
            <Input
              id="nickname"
              placeholder={t('addCreditCard.nicknamePlaceholder')}
              value={cardData.nickname || ''}
              onChange={(e) => setCardData(prev => ({ ...prev, nickname: e.target.value }))}
            />
          </div>

          {/* Billing Address */}
          <div className="space-y-4">
            <h3 className="text-lg font-semibold">{t('addCreditCard.billingAddress')}</h3>
            
            <div className="space-y-2">
              <Label htmlFor="street">{t('addCreditCard.streetAddress')}</Label>
              <Input
                id="street"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <Label htmlFor="city">{t('addCreditCard.city')}</Label>
                <Input
                  id="city"
              ...
              <div className="space-y-2">
                <Label htmlFor="state">{t('addCreditCard.state')}</Label>
                <Input
                  id="state"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <Label htmlFor="zipCode">{t('addCreditCard.zipCode')}</Label>
                <Input
                  id="zipCode"
              ...
              <div className="space-y-2">
                <Label htmlFor="country">{t('addCreditCard.country')}</Label>
                <Select 
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="US">{t('addCreditCard.unitedStates')}</SelectItem>
                    <SelectItem value="CA">{t('addCreditCard.canada')}</SelectItem>
                    <SelectItem value="UK">{t('addCreditCard.unitedKingdom')}</SelectItem>
                    <SelectItem value="AU">{t('addCreditCard.australia')}</SelectItem>
                    <SelectItem value="DE">{t('addCreditCard.germany')}</SelectItem>
                    <SelectItem value="FR">{t('addCreditCard.france')}</SelectItem>
                    <SelectItem value="JP">{t('addCreditCard.japan')}</SelectItem>
                    <SelectItem value="KR">{t('addCreditCard.southKorea')}</SelectItem>
                    <SelectItem value="CN">{t('addCreditCard.china')}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
        ...
        <div className="space-y-3 pb-8">
          <Button 
            ...
            className="w-full"
          >
            {isSaving ? t('addCreditCard.savingCard') : t('addCreditCard.saveCard')}
          </Button>
          <Button 
            variant="outline" 
            onClick={onBack}
