...
import { ArrowLeft, DollarSign, Clock, AlertCircle, CheckCircle2, MapPin, Package, User } from 'lucide-react';
...
export function MakeOfferPage({ request, onBack, onSubmitOffer }: MakeOfferPageProps) {
  const { t } = useLanguage();
  ...
  // Sample main image
  const mainImage = request.images?.[0] || "https://images.unsplash.com/photo-1726695716109-68a7321c0664?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxzbmVha2VycyUyMHNob3BwaW5nJTIwcHJvZHVjdHxlbnwxfHx8fDE3NTg2MzU5MTh8MA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral";
  ...
  const validateForm = () => {
    ...
    if (!offerDescription.trim() || offerDescription.trim().length < 50) {
  ...
  const totalAmount = (parseFloat(offerPrice) || 0) + (parseFloat(serviceFee) || 0);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="sticky top-0 z-10 bg-background border-b border-border">
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-lg font-semibold">{t('makeOffer.title')}</h1>
              <p className="text-sm text-muted-foreground">{t('makeOffer.subtitle')}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 pb-40">
        ...
        <div className="mb-6 p-4 bg-muted/50 rounded-xl">
          <div className="flex space-x-4">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-medium mb-1 truncate">{request.title}</h3>
          ...
          {request.budget && (
            <div className="mt-3 pt-3 border-t border-border">
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">{t('makeOffer.clientBudget')}</span>
                <BudgetDisplay budget={request.budget} variant="minimal" />
              </div>
            </div>
          )}
        </div>
        ...
        {/* Client Info */}
        <div className="mb-6 p-4 bg-muted/50 rounded-xl">
          <h3 className="font-medium mb-3 text-foreground">{t('makeOffer.clientInformation')}</h3>
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center space-x-2 text-sm text-muted-foreground">
                ...
                <span>•</span>
                <span>{clientData.reviews} {t('makeOffer.reviews')}</span>
                <span>•</span>
                <span>{t('makeOffer.memberSince')} {clientData.since}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <form className="space-y-6">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <Input
                ...
                className={`pl-10 bg-input-background border-border ${priceError ? 'border-destructive' : ''}`}
              />
            </div>
            {priceError && (
          ...
          <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
            <span className="font-medium">{t('makeOffer.totalAmount')}</span>
          ...
          <div>
            ...
            <Select value={estimatedTime} onValueChange={setEstimatedTime}>
              <SelectTrigger className={`bg-input-background border-border ${timeError ? 'border-destructive' : ''}`}>
                <div className="flex items-center">
                  <Clock className="h-4 w-4 text-muted-foreground mr-2" />
                  <SelectValue placeholder={t('makeOffer.selectTimeframe')} />
                </div>
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="1-2 days">{t('makeOffer.timeframe1to2Days')}</SelectItem>
                <SelectItem value="3-5 days">{t('makeOffer.timeframe3to5Days')}</SelectItem>
                <SelectItem value="5-7 days">{t('makeOffer.timeframe5to7Days')}</SelectItem>
                <SelectItem value="1-2 weeks">{t('makeOffer.timeframe1to2Weeks')}</SelectItem>
                <SelectItem value="2-3 weeks">{t('makeOffer.timeframe2to3Weeks')}</SelectItem>
                <SelectItem value="1 month">{t('makeOffer.timeframe1Month')}</SelectItem>
              </SelectContent>
            </Select>
            {timeError && (
              <p className="text-sm text-destructive flex items-center mt-1">
            ...
            <div className="p-3 bg-muted/30 rounded-lg">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm">
                  {request.deliveryMethod === 'personal' ? t('makeOffer.personalDelivery') : t('makeOffer.shipToClient')}
                </span>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <Textarea
              ...
              className={`bg-input-background border-border min-h-[120px] ${descriptionError ? 'border-destructive' : ''}`}
            />
            {descriptionError && (
              <p className="text-sm text-destructive flex items-center mt-1">
                <AlertCircle className="h-3 w-3 mr-1" />
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {offerDescription.length}/500 {t('makeOffer.charactersMinimum')}
            </p>
          </div>
