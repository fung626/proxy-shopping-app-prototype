...
import { ArrowLeft, Search, Filter, Star, MapPin, Clock, Package, CheckCircle, Users, Award, Zap } from 'lucide-react';
...
export function AllAgentsPage({ onBack, onContactAgent }: AllAgentsPageProps) {
  const { t } = useLanguage();
  ...
  const filteredAgents = useMemo(() => allAgents
    .filter(agent => {
      const matchesSearch = searchQuery === '' || 
        agent.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        agent.specialty.toLowerCase().includes(searchQuery.toLowerCase()) ||
        agent.serviceArea.toLowerCase().includes(searchQuery.toLowerCase());
      
      const matchesSpecialty = selectedSpecialty === 'all' || 
        agent.specialty.toLowerCase().includes(selectedSpecialty.toLowerCase());
      
      const matchesLocation = selectedLocation === 'all' || 
        (selectedLocation === 'north-america' && (agent.serviceArea.includes('NY') || agent.serviceArea.includes('CA') || agent.serviceArea.includes('WA'))) ||
        (selectedLocation === 'europe' && (agent.serviceArea.includes('France') || agent.serviceArea.includes('Switzerland'))) ||
        (selectedLocation === 'asia' && (agent.serviceArea.includes('Korea') || agent.serviceArea.includes('Japan')));
    ...
    return (
      ...
      <div className="bg-card px-4 pt-6 pb-4 border-b border-border">
        <div className="flex items-center space-x-3 mb-4">
          ...
          <div>
            <h1 className="text-lg font-semibold text-foreground">{t('allAgents.title')}</h1>
            <p className="text-xs text-muted-foreground">{t('allAgents.subtitle')}</p>
          </div>
        </div>
      ...
      <div className="px-4 py-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <span className="text-muted-foreground text-sm">
            {filteredAgents.length} {filteredAgents.length === 1 ? t('allAgents.agent') : t('allAgents.agents')} {t('allAgents.found')}
          </span>
        ...
        <div className="space-y-4">
          {filteredAgents.map((agent) => (
            <div key={agent.id} className="p-4 rounded-xl bg-card border border-border hover:shadow-md transition-shadow">
                ...
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-3 text-sm text-muted-foreground">
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center space-x-1">
                        ...
                        <span>{agent.completedOrders} {t('allAgents.ordersCompleted')}</span>
                      </div>
                      <div className="flex items-center space-x-1">
                        <Award className="h-3 w-3" />
                        <span>{agent.successRate}% {t('allAgents.successRate')}</span>
                      </div>
                    </div>
                    <div className="space-y-1">
                      <div className="flex items-center space-x-1">
                        <Clock className="h-3 w-3" />
                        <span>{t('allAgents.responds')} {agent.responseTime}</span>
                      </div>
                      <div className="flex items-center space-x-1">
                        <Zap className="h-3 w-3" />
                        <span>{t('allAgents.avgDelivery')}: {agent.averageDelivery}</span>
                      </div>
                      <div className="flex items-center space-x-1">
                        <Users className="h-3 w-3" />
                        <span>{t('allAgents.joined')} {new Date(agent.joinDate).getFullYear()}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {agent.languages && (
                    <div className="text-xs text-muted-foreground mb-3">
                      <span className="font-medium">{t('allAgents.languages')}:</span> {agent.languages.join(', ')}
                    </div>
                  )}
        ...
        {filteredAgents.length === 0 && (
          <div className="text-center py-8">
            ...
            <h3 className="text-lg font-medium text-foreground mb-2">{t('allAgents.noAgentsFound')}</h3>
            <p className="text-muted-foreground">{t('allAgents.tryAdjustingFilters')}</p>
          </div>
        )}
      </div>
    </div>
