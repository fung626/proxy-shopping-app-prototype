...
export function CreateOfferTab({
  user,
  onSignIn,
}: CreateOfferTabProps) {
  const { t } = useLanguage();
  ...
  const toggleDeliveryOption = (option: string) => {
    ...
    if (currentOptions.includes(option)) {
      setFormData({
        ...formData,
        deliveryOptions: currentOptions.filter(
          (opt) => opt !== option
        ),
      });
    } else {
      ...
      <div className="flex-1 bg-background pb-20">
        ...
        <div className="px-4 py-8">
          <Card className="p-6 text-center">
            <Package className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
            <h3 className="text-lg font-medium mb-2">
              {t('createOffer.agentCapabilityRequired')}
            </h3>
            <p className="text-muted-foreground mb-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            {formData.images.map((image, index) => (
              <div
                key={index}
                className="aspect-square border border-border rounded-lg bg-muted relative"
              >
                <img
                  src={image}
                  alt={`Product ${index + 1}`}
        ...
        <div>
          ...
          >
            <SelectTrigger className="bg-input-background border-border">
              <SelectValue
                placeholder={t('createOffer.selectCategory')}
              />
            </SelectTrigger>
            <SelectContent>
        ...
        <div className="grid grid-cols-1 gap-6">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <Input
                placeholder={t(
                  'createOffer.shoppingLocationPlaceholder'
                )}
                value={formData.shoppingLocation}
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    shoppingLocation: e.target.value,
                  })
                }
                className="pl-10 bg-input-background border-border"
                required
