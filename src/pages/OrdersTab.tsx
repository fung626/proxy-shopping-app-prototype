...
import { Package, Clock, CheckCircle, XCircle, X, Search, ShoppingBag, Truck, CreditCard, PackageCheck } from 'lucide-react';
...
export function OrdersTab({ user, onSignIn, onViewDetails, onViewOffers, onContactAgent, onLeaveFeedback, onViewFeedback }: OrdersTabProps) {
  const { t } = useLanguage();
  ...
  const requests = [
    {
      ...
      description: 'Looking for specific Japanese skincare brands from Tokyo including Shiseido, SK-II, and other premium brands',
      status: 'Active',
      step: 1,
      role: 'client',
      agent: null,
    ...
    },
    {
      ...
      description: 'Nike Air Jordan 1 Retro High OG from US stores - looking for size 10.5 in Chicago colorway',
    ...
    },
    {
      ...
      description: 'Authentic Louis Vuitton bag from Paris boutique - Neverfull MM in Damier Ebene',
      status: 'Completed',
      step: 5,
      role: 'client',
      agent: 'Marie Dubois',
    ...
    },
    {
      ...
      description: 'Shopping for K-beauty skincare items in Seoul - complete 10-step routine set',
    ...
    },
    {
      ...
      description: 'Finding rare vintage timepieces in Swiss markets - 1960s Omega Speedmaster',
      status: 'Completed',
      step: 4,
      role: 'agent',
    ...
    },
    {
      ...
      description: 'Looking for authentic Italian handmade leather dress shoes from Florence boutiques - size 42 EU',
      status: 'In Progress',
      step: 2,
      role: 'agent',
    ...
    },
    {
      ...
      description: 'Shopping for limited edition anime figures and merchandise from Akihabara - Studio Ghibli collection',
      status: 'purchased',
      step: 3,
      role: 'agent',
  ...
  const getStepDetails = (step: number, role: string) => {
    if (role === 'client') {
      const steps = [
        { icon: Search, label: t('orders.steps.requestPosted'), description: t('orders.steps.waitingForAgents') },
        { icon: Package, label: t('orders.steps.agentAssigned'), description: t('orders.steps.shoppingInProgress') },
        { icon: CreditCard, label: t('orders.steps.itemsPurchased'), description: t('orders.steps.itemsBoughtByAgent') },
        { icon: PackageCheck, label: t('orders.steps.packageShipped'), description: t('orders.steps.packageSentOut') },
        { icon: Truck, label: t('orders.steps.delivered'), description: t('orders.steps.orderComplete') }
      ];
      return steps[step - 1] || steps[0];
    } else {
      const steps = [
        { icon: Search, label: t('orders.steps.requestAccepted'), description: t('orders.steps.startingShopping') },
        { icon: Package, label: t('orders.steps.shoppingStarted'), description: t('orders.steps.findingRequestedItems') },
        { icon: CreditCard, label: t('orders.steps.itemsPurchased'), description: t('orders.steps.itemsBoughtSuccessfully') },
        { icon: PackageCheck, label: t('orders.steps.packageShipped'), description: t('orders.steps.packageSentToClient') },
        { icon: Truck, label: t('orders.steps.delivered'), description: t('orders.steps.successfullyCompleted') }
      ];
      return steps[step - 1] || steps[0];
    }
  };
  ...
  const getRoleLabel = (role: string) => {
    return role === 'client' ? t('orders.request') : t('orders.offer');
  };
    ...
    return (
      <div className="flex-1 bg-background pb-20">
        ...
        <div className="bg-card px-4 pt-12 pb-6">
          <h1 className="text-3xl font-semibold text-foreground">{t('nav.orders')}</h1>
        ...
        <div className="px-4 py-8">
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-foreground mb-2">{t('orders.signInPrompt')}</h3>
      ...
      <div className="px-4 py-4">
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-start space-x-3 flex-1">
                ...
                <div className="flex-1">
                  <h4 className="font-medium text-foreground mb-1">{t('orders.orderManagement')}</h4>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="p-4 text-center bg-muted/50 rounded-lg">
            ...
            <div className="text-xs text-muted-foreground">{t('orders.totalOrders')}</div>
          </div>
          <div className="p-4 text-center bg-muted/50 rounded-lg">
            <div className="text-2xl font-bold text-green-600 mb-1">
              {requests.filter(r => r.status === 'Completed').length}
            </div>
            <div className="text-xs text-muted-foreground">{t('orders.completed')}</div>
          </div>
        </div>
            ...
            return (
              <div key={request.id} className="p-4 bg-muted/50 rounded-lg">
                <div className="flex space-x-4 mb-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex justify-between items-start mb-2">
                      <Badge variant={getRoleBadgeVariant(request.role)} className="text-xs flex-shrink-0">
                        {getRoleLabel(request.role)}
                      </Badge>
                      <Badge variant={getStatusVariant(request.status)} className="ml-2 flex-shrink-0">
                ...
                <div className="mb-4">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm font-medium">{currentStepDetails.label}</span>
                    </div>
                    <span className="text-xs text-muted-foreground">
                      {t('orders.stepOf', { current: request.step, total: 5 })}
                    </span>
                  </div>
                  ...
                  {(request.isPurchased || request.isShipped) && (
                    <div className="mb-3 p-3 shopping-progress-box rounded-lg">
                      <div className="flex items-center justify-between text-xs mb-2">
                        <span className="font-medium shopping-progress-title">{t('orders.shoppingProgress')}</span>
                      </div>
                        ...
                        {request.isPurchased && (
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-2">
                              ...
                              <span className="text-xs shopping-progress-text">{t('orders.itemsPurchased')}</span>
                            </div>
                            <span className="text-xs text-muted-foreground">
                        ...
                        {request.isShipped && (
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-2">
                              ...
                              <span className="text-xs shopping-progress-blue-text">{t('orders.packageShipped')}</span>
                            </div>
                            <span className="text-xs text-muted-foreground">
                ...
                {request.role === 'client' && request.agent && (
                  <div className="mb-3">
                    <p className="text-sm text-muted-foreground">{t('orders.shoppingAgent')}</p>
                  ...
                  <div className="space-y-2">
                    <div className="flex space-x-2">
                      ...
                      <Button 
                        ...
                        size="sm" 
                        className="flex-1"
                        onClick={() => onViewFeedback(request)}
                      >
                    ...
                    {request.role === 'client' ? (
                      request.agent ? (
                        <Button 
                          ...
                          size="sm" 
                          className="flex-1"
                          onClick={() => onContactAgent(`agent_${request.id.toLowerCase()}`)}
                        >
                      ...
                      <Button 
                        ...
                        size="sm" 
                        className="flex-1"
                        onClick={() => onContactAgent(`client_${request.id.toLowerCase()}`)}
                      >
        ...
        {requests.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-foreground mb-2">{t('orders.noOrdersYet')}</h3>
