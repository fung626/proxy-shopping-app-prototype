...
import { ArrowLeft, Share2, Heart, MapPin, Calendar, DollarSign, Clock, User, Star, CheckCircle2, ChevronDown, Package, Truck, MessageCircle, Plus } from 'lucide-react';
...
interface RequestDetailsExploreViewProps {
  request: {
    id: string;
    title: string;
    description: string;
...
export function RequestDetailsExploreView({ 
  request, 
  onBack, 
  onNavigateToMakeOffer, 
  onContactAgent,
  onShareRequest 
}: RequestDetailsExploreViewProps) {
  const { t } = useLanguage();
  ...
  const displayImages = useMemo(() => 
    request.images && request.images.length > 0 ? request.images : sampleImages,
  ...
  // Mock enhanced description with more details
  const enhancedDescription = request.description || `I'm looking for premium skincare products from Seoul, specifically K-beauty items that are popular in Korea but hard to find internationally. I need authentic products with Korean packaging and preferably from popular brands like Sulwhasoo, The History of Whoo, or similar luxury lines.
...
Please include original receipts and any promotional materials or samples that come with the purchase. I'm willing to pay premium prices for genuine, high-quality products.`;
  ...
  const similarOffers = [
    ...
    },
    {
      ...
      reviews: 123,
      timeframe: '3-5 days',
      image: 'https://images.unsplash.com/photo-1586985564150-5e2f8c5b3c06?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxwcm9mZXNzaW9uYWwlMjB3b21hbiUyMGJ1c2luZXNzJTIwcG9ydHJhaXR8ZW58MXx8fHwxNzU4NjE2NjAwfDA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral'
    }
  ];
  ...
  const requestFeatures = [
    ...
    },
    {
      ...
      description: request.deliveryMethod === 'personal' ? 'Personal delivery preferred' : 'Ship to client'
    }
  ];

  const truncatedDescription = (request.description || '').slice(0, 200);
      ...
      <div className="px-4 pb-40">
        ...
        <div className="py-6">
          <div className="flex items-start justify-between mb-2">
            <h1 className="text-2xl font-semibold text-foreground pr-4">{request.title}</h1>
        ...
        {/* Client Info */}
        <div className="py-6">
          <h3 className="font-semibold mb-4">Requested by {clientData.name}</h3>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex-1">
              <div className="flex items-center space-x-2 mb-1">
                ...
                <span className="text-muted-foreground">{clientData.reviews} reviews</span>
                {clientData.verified && (
                  <>
                    <span className="text-muted-foreground">•</span>
                    <div className="flex items-center space-x-1">
              ...
              <p className="text-sm text-muted-foreground">Member since {clientData.since}</p>
            </div>
          </div>
        </div>
        ...
        <div className="py-6">
          ...
          <div className="space-y-4">
            <p className="text-foreground leading-relaxed">
              {showFullDescription ? enhancedDescription : enhancedDescription.slice(0, 300)}
              {enhancedDescription.length > 300 && !showFullDescription && '...'}
            </p>
            
            {enhancedDescription.length > 300 && (
              <Button
                variant="ghost"
        ...
        <div className="py-6">
          <h3 className="font-semibold mb-4">Specific requirements</h3>
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      <h4 className="font-medium">{offer.agentName}</h4>
            ...
            <div className="flex items-baseline space-x-1">
              <span className="text-xl font-semibold text-foreground">
                {(() => {
                  ...
                  if (typeof request.budget === 'object' && request.budget && 'min' in request.budget) {
