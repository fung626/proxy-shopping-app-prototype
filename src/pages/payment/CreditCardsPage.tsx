...
export function CreditCardsPage({ user }: CreditCardsPageProps) {
  ...
  useEffect(() => {
    ...
    console.log(
      'CreditCardsPage - User creditCards:',
      user?.creditCards
    );
    console.log(
      'CreditCardsPage - Local creditCards state:',
      creditCards
    );
  }, [user, creditCards]);
  ...
  const handleDeleteCard = (cardId: string) => {
    const cardToDelete = creditCards.find(
      (card) => card.id === cardId
    );
    const remainingCards = creditCards.filter(
      (card) => card.id !== cardId
    );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log('Saving credit cards:', creditCards);
  ...
  const handleAddCard = () => {
    navigate('/profile/credit-cards/add');
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="p-4">
              ...
              {creditCards.map((card) => (
                ...
                >
                  ...
                  <div className="text-xs text-muted-foreground">
                    <div>
                      {t('creditCards.expires')}: {card.expiryMonth}/
                      {card.expiryYear}
                    </div>
                    <div>
          ...
          <DialogContent>
            ...
            <DialogFooter className="space-x-2">
              ...
              <Button
                variant="destructive"
                onClick={() =>
                  deleteConfirmId && handleDeleteCard(deleteConfirmId)
                }
              >
        ...
        >
          {isSaving
            ? t('creditCards.saving')
            : t('creditCards.saveChanges')}
        </Button>
      </div>
    </div>
  );
